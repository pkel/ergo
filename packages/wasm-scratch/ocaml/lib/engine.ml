...
let invoke inst func arg =
  let func =
    match List.assoc_opt (Wasm.Utf8.decode func) inst.module_.exports with
    | Some (ExternFunc f) -> f
    | _ -> raise Not_found
  in
  let arg = Ejson.write inst.memory inst.alloc_p arg in
  let ret =
    match Wasm.Eval.invoke func [I32 arg] with
    | [I32 x] -> x
    | _ -> failwith "incompatible module (return type)"
  in
  Ejson.read inst.memory ret
